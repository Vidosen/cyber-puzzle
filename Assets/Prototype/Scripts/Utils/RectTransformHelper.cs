...
namespace Prototype.Scripts.Utils
{
    public static class RectTransformHelper
    {
        ...
        {
            return new Vector2(rows * childRect.width + (rows + 1) * childrenOffset,
                columns * childRect.height + (columns + 1) * childrenOffset);
        }

        public static Vector2 GetChildPsotionContainer(Rect childRect, int xIndex, int yIndex, float childrenOffset)
        {
